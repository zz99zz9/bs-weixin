...
var vmAvatar = avalon.define({
    ...
    getPic: function() {
        var pathName = document.referrer;
        var a = Storage.get("head");
        if (pathName.indexOf("subAdd") != -1 && a.headImg == defaultHeadImg) {
            vmAvatar.headImg = defaultHeadImg;
            
        } else {
            if (pathName.indexOf("subAdd") != -1 || pathName.indexOf("favoree") != -1 || pathName.indexOf("editFund") != -1) {
                vmAvatar.headImg = urlAPINet + a.headImg.replace('100.', 'ZIP.');
            } else {
                vmAvatar.headImg = urlAPINet + Storage.getLocal('user').headImg.replace('100.', 'ZIP.');
            }
        }
        console.log(vmAvatar.headImg);
    },
});
